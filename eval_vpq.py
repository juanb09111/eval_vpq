...
#%%
# Contains PQ value per category
class PQStatCat():
    def __init__(self):
        self.iou = 0.0
        self.tp = 0
...
# Contains PQ value of all classes. Uses PQStatCat
class PQStat():
    def __init__(self):
...
# TODO: Define categories format, nframes?
def vpq_compute_single_core(gt_pred_set, categories, nframes=2):
    ...
    # Iterate over whole video until T-λ
    # Iterate over the video frames 0::T-λ
    # print(len(gt_pred_set))
    for idx in range(0, len(gt_pred_set)-nframes+1): 
        # print(nframes)
        vid_pan_gt, vid_pan_pred = [], [] # list of frames
        gt_segms_list, pred_segms_list = [], [] # list of segments

        
        # Matching nframes-long tubes.
        # Collect tube IoU, TP, FP, FN
        # gt_pred_set returns = gt_json, pred_json, gt_pan, pred_pan, gt_image_json
        for i, (gt_json, pred_json, gt_pan, pred_pan, gt_image_json) in enumerate(gt_pred_set[idx:idx+nframes]):
            ...
            gt_pan, pred_pan = np.uint32(gt_pan), np.uint32(pred_pan) # panoptic images 
            
            # pan_gt = gt_pan[:, :, 0] + gt_pan[:, :, 1] * 256 + gt_pan[:, :, 2] * 256 * 256 #HxWx3 --> HxWx1
            # pan_pred = pred_pan[:, :, 0] + pred_pan[:, :, 1] * 256 + pred_pan[:, :, 2] * 256 * 256 #HxWx3 --> HxWx1
            # pan_pred = pred_pan[:, :, 0]  + pred_pan[:, :, 1] *0  + pred_pan[:, :, 2] * 0 #HxWx3 --> HxWx1
            pan_gt = gt_pan
            pan_pred = pred_pan
            # gt_segms = {"id_0123": {'area': 546, attr2: , attr3: }}
            # Sum areas corresponding to same el id
            # print(len(pred_json["segments_info"]))
            gt_segms = {}
    ...
    #         # predicted segments area calculation + prediction sanity checks
            pred_labels_set = set(el['id'] for el in pred_json['segments_info']) # set of segments ids
            labels, labels_cnt = np.unique(pan_pred, return_counts=True) # labels and number of appearances per label
            # print(labels, len(labels))
            # print(pred_labels_set)
            for label, label_cnt in zip(labels, labels_cnt):
                if label not in pred_segms: # if label from pan_pred is not in pred_segms
                    if label == VOID:
                        continue
                    pred_pan[pred_pan!=1] = 0
                    plt.imshow(pred_pan, interpolation='nearest')
                    plt.show()
            ...
            vid_pan_gt.append(pan_gt) #vid_pan_gt = list of nframes frames (nframesxHxW)
            vid_pan_pred.append(pan_pred)
            gt_segms_list.append(gt_segms) # list of nframes segments
        ...
        # while gt_segms_list and pred_segms_list are lists of segments...
        # vid_gt_segms and vid_pred_segms are objects that agregate across all the elements (nframes) in gt_segms_list and pred_segms_list
        ...
        # print(vid_pred_segms)
        #### Step3. Confusion matrix calculation
        vid_pan_gt_pred = vid_pan_gt.astype(np.uint64) * OFFSET + vid_pan_pred.astype(np.uint64) # [nf,H,W]*OFFSET + [nf,H,W]
        gt_pred_map = {}
        labels, labels_cnt = np.unique(vid_pan_gt_pred, return_counts=True) 
        for label, intersection in zip(labels, labels_cnt):
            gt_id = label // OFFSET
            pred_id = label % OFFSET
            # print("gt_id", gt_id, pred_id)
            gt_pred_map[(gt_id, pred_id)] = intersection #gt_label = 11 pred_label= 20 get_pred_map[11, 20] = N
        ...
        # false negatives are all the gt_labels not matched with a threshold of 0.5 
        crowd_labels_dict = {}
...
def main():
    ...
    # print("gt_categorires", categories)
    ...
    files = [item['file_name'].replace('.jpg','.png') for item in gt_jsons['images']]
    # print(files)

    # TODO: sort images CAUTION!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    # files.sort()
    ...
    start_time = time.time()
    pred_pans = []
    files = [item['file_name'].replace('.jpg', '.png')for item in pred_jsons['images']]
